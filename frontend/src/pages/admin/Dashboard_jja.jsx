// ============================================================
// Dashboard_jja.jsx — Panel de Control del Administrador
...
const COLORES_AVATAR = ['#0056b3', '#00bcd4', '#ff9800', '#ef4444', '#9c27b0', '#3b82f6', '#ec4899', '#06b6d4']
...
const Dashboard_jja = () => {
  ...
  const [kpis_jja, setKpis_jja] = useState({ activosDisponibles: 0, prestamos: 0, solicitudes: 0, usuarios: 0 })
  ...
  async function cargarDatos_jja() {
    try {
      const [activosResp, prestamosResp, solicitudesResp, usuariosResp] = await Promise.allSettled([
        apiRequest('/activos'),
      ...
      const usuarios = extraerLista_jja(usuariosResp)

      // KPIs — "Activos Disponibles"
      ...
      // Últimos préstamos del día (hoy)
      const hoy_jja = new Date()
      const inicioHoy_jja = new Date(hoy_jja.getFullYear(), hoy_jja.getMonth(), hoy_jja.getDate())
      const prestamosHoy_jja = prestamos.filter(p => {
        const fecha = new Date(p.creado_en_jja || p.fecha_prestamo_jja)
        return fecha >= inicioHoy_jja
      })
      ...
      // Solicitudes recientes (máximo 5)
      setSolicitudesRecientes_jja(
        solicitudes
          .sort((a, b) => new Date(b.fecha_solicitud_jja || b.creado_en_jja || Date.now()) - new Date(a.fecha_solicitud_jja || a.creado_en_jja || Date.now()))
          .slice(0, 5)
      )
    } catch (err) {
  ...
  // Helper para extraer lista de respuestas Promise.allSettled
  function extraerLista_jja(resultado) {
    if (!resultado || resultado.status !== 'fulfilled') return []
    const data = resultado.value
    
    // Si la API devuelve el objeto de un solo elemento (e.g. un solo usuario), envuelvelo en array
    ...
    // Si es un objeto único directamente retornado (como Perfil de usuario)
  ...
  // Color de avatar por hash del nombre
  ...
  return (
    <div>
      ...
      <div className="pagina-header-jja">
        <div>
          ...
          <p className="pagina-subtitulo-jja">
            Bienvenido al sistema de {tema.nombreEmpresa} · {fechaCapitalizada}
          </p>
        </div>
      </div>
      ...
      <div className="acciones-rapidas-jja">
        ...
        <BotonAccion_jja variante="exito" icono={<IconoUsuarios_jja />} onClick={() => navigate('/sistema/usuarios')}>
          Gestión de Usuarios
        </BotonAccion_jja>
                  ...
                  return (
                    <li key={p.id_prestamo_jja || i} className="mini-lista-item-jja">
                      <div className="mini-lista-izq-jja">
                        ...
                        {usuarioImg ? (
                          <img src={usuarioImg} alt="Usuario" style={{ width: 44, height: 44, borderRadius: '50%', objectFit: 'cover', flexShrink: 0, border: '2px solid var(--border-color-jja)' }} />
                        ) : (
                  ...
                  return (
                    <li key={s.id_solicitud_jja || i} className="mini-lista-item-jja">
                      <div className="mini-lista-izq-jja">
                        {/* Avatar del cliente */}
                        {clienteImg ? (
                          <img src={clienteImg} alt="Cliente" style={{ width: 44, height: 44, borderRadius: '50%', objectFit: 'cover', flexShrink: 0, border: '2px solid var(--border-color-jja)' }} />
                        ) : (
