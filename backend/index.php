...
// ── 4. CORS — Solo el origen autorizado puede hacer peticiones ──
$corsOrigin_jja = $_ENV['CORS_ORIGIN'] ?? 'http://localhost:5173';
$originHeader_jja = $_SERVER['HTTP_ORIGIN'] ?? '';

// Permitir el origen configurado (en dev puede ser *, en prod el dominio exacto)
if ($originHeader_jja === $corsOrigin_jja || $_ENV['APP_ENV'] === 'development') {
    header("Access-Control-Allow-Origin: {$corsOrigin_jja}");
} else {
    header("Access-Control-Allow-Origin: {$corsOrigin_jja}");
}
